...
import './styles.css'; // Import the CSS file if necessary
  ...
  return (
    <div className={`${state.toggleNavbar ? 'block' : 'hidden'}`}>
      ...
      <div className="h-full z-50 fixed drop-shadow-2xl md:drop-shadow flex">
            ...
            <div
              ...
              className={`p-3.5 cursor-pointer ${
                activeIcon === icon.icon
                  ? 'text-white bg-gradient-to-bl bg-green-400 to-pink-500'
                  : 'text-neutral-400'
        ...
        <div
          className={`w-56 ${
            state.toggle ? 'animate-slideIn' : 'animate-slideOut'
          ...
          {category.map(({ text, icon, url }, index) => (
            <Link
              ...
              className={`self-stretch duration-300 cursor-pointer px-[18px] ${
                activeCat === text
                  ? 'bg-orange-50 bg-opacity-80 border-r-2 border-green-600'
