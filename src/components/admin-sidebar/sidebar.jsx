                ...
                <div className="flex-col overflow-hidden md:overflow-auto justify-start items-start gap-4 flex bg-zinc-900 px-4 py-6 min-h-full">
                    {
                        Data.map((icon, index,) => {
                            return (
                                <div key={index} onClick={(r) => { ClickedIcon(icon) }} className={`p-3.5 cursor-pointer ${activeIcon === icon.icon ? ' text-white bg-gradient-to-bl bg-green-400 to-pink-500' : 'text-neutral-400'}  rounded-lg flex-col hover:text-white duration-300 justify-start items-center gap-2 flex`}>
                        ...
                        category.map(({ text, icon, url }, index) => {
                            return (
                                <Link to={`/admin-dashboard/${url}`} onClick={() => {
                                    ...
                                    <div className={`grow shrink basis-0 ${activeCat === text ? 'text-green-400 font-semibold' : 'text-zinc-500  font-normal'} text-sm leading-tight`}>{text}</div>
                                </Link>
                            )
                        })
                    }
